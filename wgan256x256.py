...
class ResidualBlock(nn.Module):
    def __init__(self, input_dim, output_dim, kernel_size, resample=None, hw=DIM):
        ...
        #print('---within ResidualBlock __init__')
...
class GoodGenerator(nn.Module):
    def __init__(self, dim=DIM, output_dim=OUTPUT_DIM):
        ...
        self.ln1 = nn.Linear(1024, 4*4*8*self.dim)
        self.rb1 = ResidualBlock(8*self.dim, 8*self.dim, 3, resample='up')
        self.rb2 = ResidualBlock(8*self.dim, 4*self.dim, 3, resample='up')
        self.rb3 = ResidualBlock(4*self.dim, 2*self.dim, 3, resample='up')
        self.rb4 = ResidualBlock(2*self.dim, 1*self.dim, 3, resample='up')
        self.rb5 = ResidualBlock(1*self.dim, int(1/2*self.dim), 3, resample='up')
        self.rb6 = ResidualBlock(int(1/2*self.dim), int(1/4*self.dim), 3, resample='up')
        self.bn = nn.BatchNorm2d(int(1/4*self.dim))

        self.conv1 = MyConvo2d(int(1/4*self.dim), 1, 3)
        self.relu = nn.ReLU()
        self.tanh = nn.Tanh()
        self.initialize()
...
class GoodDiscriminator(nn.Module):
    def __init__(self, dim=DIM):
        ...
        self.conv1 = MyConvo2d(1, int(1/4*self.dim), 3, he_init=False)
        self.rb1 = ResidualBlock(int(1/4*self.dim), int(1/2*self.dim),
                                 3, resample='down', hw=DIM)
        self.rb2 = ResidualBlock(
            int(1/2*self.dim), self.dim, 3, resample='down', hw=int(DIM/2))
        self.rb3 = ResidualBlock(
            self.dim, 2*self.dim, 3, resample='down', hw=int(DIM/4))
        self.rb4 = ResidualBlock(
            2*self.dim, 4*self.dim, 3, resample='down', hw=int(DIM/8))
        self.rb5 = ResidualBlock(
            4*self.dim, 8*self.dim, 3, resample='down', hw=int(DIM/16))
